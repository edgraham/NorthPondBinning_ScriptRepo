#! /bin/bash
##Binning workflow used for NorthPond Metagenomes
fasta=$1 #Assembly/Fasta File name
coverage_file=$2 #Location of coverage file with full path
location_of_fasta_file=$3 #Location of fasta file give full path leading to directory
...
if [ "$(ls -A strain_redundancy)" ]; then
        mv strain_redundancy/*.fna high_completion
fi
rm -rf strain_redundancy
...
#########PASS7########
	...
	if [ "$(ls -A ../PASS6/strain_redundancy/)" ]; then
        	cp ../PASS6/strain_redundancy/*.fna ../Final-Genomes
        fi
	cd ../
        mkdir Binsanity-Records
        mv PASS* Binsanity-Records
